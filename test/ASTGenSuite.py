...
class ASTGenSuite(unittest.TestCase):
    ...
    def test_method_invocation(self):
        ...
        expect = str(Program([
            ClassDecl(Id("B"),[
                MethodDecl(Id("A"),[],IntType(),Block([
                    CallStmt(Id("Shape"),Id("@getNumOfShape"),[])
                ]))
            ],Id("A"))
        ]))
                ...
                MethodDecl(Id("@main"),[
                    ...
                    For(
                        ...
                        Block([
                            ...
                            CallStmt(Id("io"),Id("@writeStr"),[StringLiteral("Press 1 to exit: ")]),
                            If(
        ...
        expect = str(Program([
            ClassDecl(Id("Program"),[
                MethodDecl(Id("@main"),[],IntType(),Block([
                    ...
                    CallStmt(Id("io"),Id("@writeStr"),[Id("str")]),
                    VarDecl(Id("escapeStr"),StringType(),StringLiteral("First Line")),
                    CallStmt(Id("io"),Id("@writeStr"),[Id("escapeStr")])
                ]))
            ])
        ]))
    ...
    def test_array_lit(self):
        ...
        expect = str(Program([
            ClassDecl(Id("A"),[
                AttributeDecl(VarDecl(Id("lst"),ArrayType(5, IntType()),ArrayLiteral([IntLiteral(1),IntLiteral(2),IntLiteral(3)]))),
                MethodDecl(Id("constructor"),[],VoidType(),Block([
                    CallStmt(Id("io"),Id("@writeInt"),[FieldAccess(SelfLiteral(),Id("lst"))])
                ]))
            ]),
            ClassDecl(Id("Program"),[
                MethodDecl(Id("@main"),[],IntType(),Block([
                    VarDecl(Id("obj"),ClassType(Id("A")),NewExpr(Id("A"),[])),
                    CallStmt(Id("io"),Id("@writeInt"),[FieldAccess(Id("obj"),Id("lst"))])
                ]))
            ])
        ]))
